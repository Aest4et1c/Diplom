...
/* admin/profile/edit.php — воспитатель редактирует СВОЮ новость
   ▸ Полный функционал как у admin/news/edit.php:
       — замена / удаление обложки,
       — редактирование подписи существующих изображений,
       — замена файла или удаление картинки,
       — добавление новых изображений с live‑превью.
   ▸ Доступ:
       — role = teacher,
       — news.staff_id = $user['staff_id'].
*/
...
$art = $art->fetch();
if(!$art)                   exit('404');
if($art['staff_id'] != $u['staff_id']) exit('403');      // чужая новость
