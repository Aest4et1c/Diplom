 ...
 * 0 = черновик (виден только администратору); 1 = опубликовано
 * • Клик по опубликованной → article.php?id
 * • Клик по черновику (только у админа) → /​admin/news/edit.php?id
 */
...
$totalSql = "SELECT COUNT(*) FROM articles".(!$isAdmin ? " WHERE status = 1" : "");
...
$sql = "
    SELECT id, title, cover_image, created_at, status,
           SUBSTRING(body,1,250) AS excerpt
      FROM articles";
if(!$isAdmin) $sql .= " WHERE status = 1";
$sql .= "
...
<style>
   .news-card:hover {text-decoration:none; box-shadow:0 0.5rem 1rem rgba(0,0,0,.15);}
...
<main class="container my-4 flex-grow-1">
 ...
 <?php else: ?>
   ...
   <?php foreach($articles as $a):
         /* ссылка: черновик админа → редактор, иначе статья */
